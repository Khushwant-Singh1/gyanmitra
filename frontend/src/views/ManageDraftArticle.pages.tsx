    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col space-y-2">
            <Label htmlFor="message">Add message for request (Optional)</Label>
            <Textarea
              id="message"
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder="Write a message to the receiver (optional)..."
              className="w-full"
            />
          </div>
        </div>
          ...
          <Button
            onClick={() => {
              if (!receiverId || receiverId.trim().length === 0)
                return toast.error('Please select a receiver');
              open = false;
              onOpenChange(open);
...
export const DraftArticle: React.FC = () => {
  ...
  const queryClient = useQueryClient();
  const userMe = (
    (queryClient.getQueryData(['me']) as any)?.data as IApiCurrentUserSession | undefined
  )?.user;
