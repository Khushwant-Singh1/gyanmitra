import { a as __generator, c as createError, f as nullthrows, i as __extends, n as __assign, r as __awaiter } from "./utils-52664384-BFyeeTtW.js";
import { t as SandpackClient } from "./base-80a1f760-CKX8Hxl8.js";
import { a as generateRandomId, c as writeBuffer, i as fromBundlerFilesToFS, n as consoleHook, o as getMessageFromError, r as findStartScriptPackageJson, s as readBuffer, t as EventEmitter } from "./consoleHook-59e792cb-f6IT6Y6f.js";
//#region node_modules/.pnpm/@codesandbox+nodebox@0.1.8/node_modules/@codesandbox/nodebox/build/index.mjs
...
var require_fingerprint_browser = __commonJS({ "../../node_modules/.pnpm/cuid@2.1.8/node_modules/cuid/lib/fingerprint.browser.js"(exports, module) {
	var pad = require_pad();
	var globalCount = Object.keys(typeof window === "object" ? window : self).length;
	var clientId = pad(((navigator.mimeTypes ? navigator.mimeTypes.length : 0) + navigator.userAgent.length).toString(36) + globalCount.toString(36), 4);
	...
	var _Emitter = class {
		...
		prependListener(eventName, listener) {
			...
			if (listeners.length > 0) {
				...
				__privateGet(this, _events).set(eventName, nextListeners);
			} else __privateGet(this, _events).set(eventName, listeners.concat(listener));
			return this;
		}
		...
		removeAllListeners(eventName) {
			...
			else __privateGet(this, _events).clear();
			return this;
		}
		listeners(eventName) {
...
invariant.as = (ErrorConstructor, predicate, message, ...positionals) => {
	if (!predicate) throw ErrorConstructor.prototype.name != null ? new ErrorConstructor(format(message, positionals)) : ErrorConstructor(format(message, positionals));
};
...
var MessageSender = class {
	...
	async handshake() {
		...
		const rejectionTimeout = setTimeout(() => {
			handshakePromise.reject(/* @__PURE__ */ new Error("MessageSender: Handshake timeout"));
		}, 5e3);
	...
	on(event, listener, options) {
		...
		this.emitter.addEventListener(event, (message) => {
			if (message instanceof MessageEvent) listener(message);
		}, options);
	}
...
var FileSystemApi = class {
	...
	async writeFile(path, content, options) {
		...
		} else if (typeof options === "string") encoding = options;
...
var ShellProcess = class {
	...
	forwardStdEvents() {
		this.channel.on("worker/tty", (message) => {
			const { data } = message;
			if (data.workerId !== this.id) return;
		...
		switch (message) {
			...
			case "exit":
				this.channel.on("worker/exit", ({ data }) => {
					if (data.workerId === this.id) listener(data.exitCode, data.error);
				});
				return;
		}
	}
...
var PreviewApi = class {
	...
	async waitFor(payload, predicate, timeout = TIMEOUT) {
		...
		if (foundPreview) readyPromise.resolve({
		...
		this.channel.on("preview/port/ready", ({ data }) => {
			if (!foundPreview && predicate(data)) readyPromise.resolve({
				url: data.url,
				port: data.port,
...
var Nodebox = class {
	...
	async connect() {
		...
		if (!this.url) connectionPromise.reject(/* @__PURE__ */ new Error("Nodebox URL is missing. Did you forget to provide it when creating this Nodebox instance?"));
	...
	get fs() {
		...
		if (this.fileSystemApi) return this.fileSystemApi;
	...
	get shell() {
		...
		if (this.shellApi) return this.shellApi;
	...
	get preview() {
		...
		if (this.previewApi) return this.previewApi;
...
//#endregion
//#region node_modules/.pnpm/@codesandbox+sandpack-client@2.19.8/node_modules/@codesandbox/sandpack-client/dist/clients/node/index.mjs
		...
		return __generator(this, function(_a) {
			...
			return [2, new Promise(function(resolve, reject) {
				...
				iframe.addEventListener("error", function() {
					return reject(/* @__PURE__ */ new Error("Iframe error"));
				});
				iframe.addEventListener("abort", function() {
					return reject(/* @__PURE__ */ new Error("Aborted"));
				});
				triesToSetUrl();
			})];
		});
...
function setupHistoryListeners(_a) {
	...
	function handleMessage(_a) {
		var data = _a.data;
		if (data.type === "urlback") history.back();
		else if (data.type === "urlforward") history.forward();
		else if (data.type === "refresh") document.location.reload();
	}
...
function watchResize(_a) {
	...
	function sendResizeEvent() {
		...
		if (lastHeight !== height) window.parent.postMessage({
			type: "resize",
			height,
			codesandbox: true,
			channelId: scope.channelId
...
var injectScriptToIframe = function(iframe, channelId) {
	scripts.forEach(function(_a) {
		...
		(_b = iframe.contentWindow) === null || _b === void 0 || _b.postMessage(message, "*");
	});
};
var SandpackNode = function(_super) {
	...
	SandpackNode.prototype._init = function(files) {
		return __awaiter(this, void 0, void 0, function() {
			return __generator(this, function(_a) {
				switch (_a.label) {
					case 0: return [4, this.emulator.connect()];
					case 1:
						_a.sent();
			...
			return __generator(this, function(_a) {
				switch (_a.label) {
					case 0:
						...
						if (!this._initPromise) this._initPromise = this._init(files);
					...
					case 1:
						...
						return [4, this.emulatorShellProcess.on("progress", function(data) {
							...
							if (data.state === "command_running" || data.state === "starting_command") {
								_this.dispatch({
									type: "shell/progress",
									data: __assign(__assign({}, data), { command: [(_a = _this.emulatorCommand) === null || _a === void 0 ? void 0 : _a[0], (_b = _this.emulatorCommand) === null || _b === void 0 ? void 0 : _b[1].join(" ")].join(" ") })
								});
	...
	SandpackNode.prototype.manageIframes = function(selector) {
		...
		*/
		if (typeof selector === "string") {
			...
			element === null || element === void 0 || element.appendChild(this.iframe);
		...
		*/
		...
		(_a = this.iframe.parentNode) === null || _a === void 0 || _a.appendChild(this.emulatorIframe);
	};
	...
	*/
	SandpackNode.prototype.dispatchDoneMessage = function() {
		...
		if (this.iframePreviewUrl) this.dispatch({
			type: "urlchange",
				...
				switch (_a.label) {
					case 0:
						window.addEventListener("message", function(event) {
							if (event.data.type === "PREVIEW_LOADED") injectScriptToIframe(_this.iframe, _this.messageChannelId);
							if (event.data.type === "urlchange" && event.data.channelId === _this.messageChannelId) _this.dispatch({
								type: "urlchange",
								url: event.data.url,
								back: event.data.back,
								forward: event.data.forward
							});
							else if (event.data.channelId === _this.messageChannelId) _this.dispatch(event.data);
						});
								...
								return __generator(this, function(_b) {
									switch (_b.label) {
										...
										case 2:
											...
											_a = event.type;
											switch (_a) {
												case "change": return [3, 3];
												case "create": return [3, 3];
												case "remove": return [3, 5];
												case "rename": return [3, 6];
												case "close": return [3, 8];
											}
											return [3, 9];
										case 3: return [4, this.emulator.fs.readFile(event.path, "utf8")];
										case 4:
											content = _b.sent();
											this.dispatch({
												type: "fs/change",
		...
		return __awaiter(this, void 0, void 0, function() {
			return __generator(this, function(_b) {
				switch (_b.label) {
					...
					case 1:
						_b.sent();
						(_a = this.iframe) === null || _a === void 0 || _a.removeAttribute("attr");
		...
		*/
		if (((_a = this.emulatorShellProcess) === null || _a === void 0 ? void 0 : _a.state) === "running") {
			Object.entries(modules).forEach(function(_a) {
				...
				if (!_this._modulesCache.get(key) || readBuffer(value) !== readBuffer(_this._modulesCache.get(key))) _this.emulator.fs.writeFile(key, value, { recursive: true });
			});
			return;
		}
		/**
			...
			return __generator(this, function(_d) {
				switch (_d.label) {
					case 0:
						_c = message.type;
						switch (_c) {
							case "compile": return [3, 1];
							case "refresh": return [3, 2];
							case "urlback": return [3, 4];
							case "urlforward": return [3, 4];
							case "shell/restart": return [3, 5];
							case "shell/openPreview": return [3, 6];
						}
						return [3, 7];
					case 1:
						this.compile(message.modules);
						return [3, 8];
					case 2: return [4, this.setLocationURLIntoIFrame()];
					case 3:
						_d.sent();
						return [3, 8];
					case 4:
						(_b = (_a = this.iframe) === null || _a === void 0 ? void 0 : _a.contentWindow) === null || _b === void 0 || _b.postMessage(message, "*");
						return [3, 8];
					case 5:
