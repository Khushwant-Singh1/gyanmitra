import { c as createError, p as invariant } from "./utils-52664384-BFyeeTtW.js";
//#region node_modules/.pnpm/@codesandbox+sandpack-client@2.19.8/node_modules/@codesandbox/sandpack-client/dist/consoleHook-59e792cb.mjs
	...
	while (current < input.length) {
		...
		switch (true) {
			...
			case isString(currentChar):
				...
				break;
			default: throw new Error("Unknown character: ".concat(currentChar));
		}
	}
	return tokens;
}
	...
	var _loop_1 = function(index) {
		if (possibleKeys[index] in scripts) {
			...
			tokenize(candidate).forEach(function(item) {
				...
				if (item.type === TokenType.EnvVar) env_1 = item.value;
				if (item.type === TokenType.Command && commandNotFoundYet) command_1 = item.value;
				if (item.type === TokenType.Argument || !commandNotFoundYet && item.type === TokenType.Command) args_1.push(item.value);
			});
			return { value: [
				command_1,
				args_1,
...
//#endregion
export { generateRandomId as a, writeBuffer as c, fromBundlerFilesToFS as i, consoleHook as n, getMessageFromError as o, findStartScriptPackageJson as r, readBuffer as s, EventEmitter as t };
