...
var require_numeral = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/*! @preserve
	* numeral.js
	* version : 2.0.6
	* author : Adam Draper
	...
	(function(global, factory) {
		...
		************************************/
		...
		numeral = function(input) {
			...
			else if (typeof input === "string") if (options.zeroFormat && input === options.zeroFormat) value = 0;
		...
		numeral._ = _ = {
			numberToFormat: function(value, format, roundingFunction) {
				var locale = locales[numeral.options.currentLocale], negP = false, optDec = false, leadingCount = 0, abbr = "", trillion = 0xe8d4a51000, billion = 1e9, million = 1e6, thousand = 1e3, decimal = "", neg = false, abbrForce, abs, int, precision, signed, thousands, output;
				value = value || 0;
				abs = Math.abs(value);
				...
				else if (signed >= 0) output = signed === 0 ? (neg ? "-" : "+") + output : output + (neg ? "-" : "+");
			...
			*/
			correctionFactor: function() {
				return Array.prototype.slice.call(arguments).reduce(function(accum, next) {
		...
		numeral.validate = function(val, culture) {
			...
			if (!val.match(/[^\d.,]/g)) {
				...
				else if (_valArray.length < 2) return !!_valArray[0].match(/^\d+.*\d$/) && !_valArray[0].match(_thousandRegEx);
				else if (_valArray[0].length === 1) return !!_valArray[0].match(/^\d+$/) && !_valArray[0].match(_thousandRegEx) && !!_valArray[1].match(/^\d+$/);
		...
		************************************/
		...
		(function() {
			...
			var unformatRegex = decimal.suffixes.concat(binary.suffixes.filter(function(item) {
		...
		(function() {
			numeral.register("format", "exponential", {
				...
				format: function(value, format, roundingFunction) {
					var output, parts = (typeof value === "number" && !numeral._.isNaN(value) ? value.toExponential() : "0e+0").split("e");
