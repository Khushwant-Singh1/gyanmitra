...
import { a as __generator, c as createError, f as nullthrows, i as __extends, l as createPackageJSON, m as dequal, n as __assign, o as __spreadArray, r as __awaiter, s as addPackageJSONIfNeeded, t as SandpackLogLevel, u as extractErrorDetails } from "./utils-52664384-BFyeeTtW.js";
...
*/
//#endregion
//#region node_modules/.pnpm/@codesandbox+sandpack-client@2.19.8/node_modules/@codesandbox/sandpack-client/dist/clients/runtime/index.mjs
...
var IFrameProtocol = function() {
	function IFrameProtocol(iframe, origin) {
		...
		if (typeof window !== "undefined") window.addEventListener("message", this.eventListener);
	}
	...
	IFrameProtocol.prototype.eventListener = function(evt) {
		if (evt.source !== this.frameWindow) return;
		var message = evt.data;
		if (!message.codesandbox) return;
		Object.values(this.globalListeners).forEach(function(listener) {
			return listener(message);
		});
		if (message.$id !== this.channelId) return;
...
for (var _i = 0, entries_1 = entries; _i < entries_1.length; _i++) {
	...
	if (extensions.length) for (var _b = 0, extensions_1 = extensions; _b < extensions_1.length; _b++) {
...
function getTemplate(pkg, modules) {
	...
	if (totalDependencies.indexOf("@nestjs/core") > -1 || totalDependencies.indexOf("@nestjs/common") > -1) return "nest";
	...
	if (totalDependencies.indexOf("react-scripts-ts") > -1) return "create-react-app-typescript";
	...
	if (totalDependencies.indexOf("@sveltech/routify") > -1 || totalDependencies.indexOf("@roxi/routify") > -1) return "node";
	...
	if (Object.keys(dependencies).length >= MAX_CLIENT_DEPENDENCY_COUNT) return "node";
}
...
var SandpackRuntime = function(_super) {
	...
	function SandpackRuntime(selector, sandboxSetup, options) {
		...
		_this.unsubscribeGlobalListener = _this.iframeProtocol.globalListen(function(mes) {
			if (mes.type !== "initialized" || !_this.iframe.contentWindow) return;
			_this.iframeProtocol.register();
			if (_this.options.fileResolver) _this.fileResolverProtocol = new Protocol("fs", function(data) {
				return __awaiter(_this, void 0, void 0, function() {
					return __generator(this, function(_a) {
						if (data.method === "isFile") return [2, this.options.fileResolver.isFile(data.params[0])];
						else if (data.method === "readFile") return [2, this.options.fileResolver.readFile(data.params[0])];
		...
		_this.unsubscribeChannelListener = _this.iframeProtocol.channelListen(function(mes) {
			switch (mes.type) {
				...
				case "status":
					...
					break;
				case "action":
					if (mes.action === "show-error") _this.errors = __spreadArray(__spreadArray([], _this.errors, true), [extractErrorDetails(mes)], false);
					break;
				case "done":
					_this.status = "done";
					break;
		...
		if (options.experimental_enableServiceWorker) _this.serviceWorkerHandshake();
		return _this;
	}
	SandpackRuntime.prototype.createBundlerURL = function() {
		var _a;
		var bundlerURL = this.options.bundlerURL || BUNDLER_URL;
		if (this.options.bundlerURL) return bundlerURL;
		if (this.options.teamId) bundlerURL = bundlerURL.replace("https://", "https://" + this.options.teamId + "-") + "?cache=".concat(Date.now());
		...
		} else bundlerURL = bundlerURL.replace(SUFFIX_PLACEHOLDER, "");
		return bundlerURL;
	};
	SandpackRuntime.prototype.serviceWorkerHandshake = function() {
		...
		if (!iframeContentWindow) throw new Error("Could not get iframe contentWindow");
		var port = channel.port1;
		port.onmessage = function(evt) {
			if (typeof evt.data === "object" && evt.data.$channel === CHANNEL_NAME) switch (evt.data.$type) {
				...
				switch (_a.label) {
					...
					case 3:
						...
						if (!headers["Content-Type"]) {
							...
							if (foundMimetype) headers["Content-Type"] = foundMimetype;
						}
						responseMessage = {
							$channel: CHANNEL_NAME,
							$type: "preview/response",
	...
	SandpackRuntime.prototype.setLocationURLIntoIFrame = function() {
		...
		(_a = this.iframe.contentWindow) === null || _a === void 0 || _a.location.replace(urlSource);
	...
	SandpackRuntime.prototype.updateSandbox = function(sandboxSetup, isInitializationCompile) {
		...
		this.dispatch(__assign(__assign({}, this.options), {
			type: "compile",
			codesandbox: true,
			version: 3,
	...
	SandpackRuntime.prototype.dispatch = function(message) {
		...
		*/
		if (message.type === "refresh") {
			...
			if (this.options.experimental_enableServiceWorker) this.serviceWorkerHandshake();
		}
	...
	*/
	...
	SandpackRuntime.prototype.getFiles = function() {
		...
		if (sandboxSetup.files["/package.json"] === void 0) return addPackageJSONIfNeeded(sandboxSetup.files, sandboxSetup.dependencies, sandboxSetup.devDependencies, sandboxSetup.entry);
