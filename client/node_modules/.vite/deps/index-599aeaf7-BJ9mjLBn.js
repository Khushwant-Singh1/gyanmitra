...
import { a as __generator, i as __extends, r as __awaiter } from "./utils-52664384-BFyeeTtW.js";
import { t as SandpackClient } from "./base-80a1f760-CKX8Hxl8.js";
import { a as generateRandomId, i as fromBundlerFilesToFS, n as consoleHook, t as EventEmitter } from "./consoleHook-59e792cb-f6IT6Y6f.js";
...
*/
//#endregion
//#region node_modules/.pnpm/@codesandbox+sandpack-client@2.19.8/node_modules/@codesandbox/sandpack-client/dist/index-599aeaf7.mjs
var import_main = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var require_db = __commonJS({ "node_modules/.pnpm/mime-db@1.52.0/node_modules/mime-db/db.json"(exports$1, module2) {
		module2.exports = {
	...
	var require_mime_db = __commonJS({ "node_modules/.pnpm/mime-db@1.52.0/node_modules/mime-db/index.js"(exports$2, module2) {
	...
	for (const [mimetype, entry] of entries) {
		...
		if (extensions?.length) for (const ext of extensions) extensionMap.set(ext, mimetype);
	}
	var EXTENSIONS_MAP = extensionMap;
	var counter = 0;
	function generateRandomId() {
		return (+`${Date.now()}${Math.round(Math.random() * 1e4)}${counter += 1}`).toString(16);
	}
	...
	var PreviewController = class {
		...
		async getIndexAtPath(filepath) {
			for (const index of this.indexFiles) try {
				return await this.getFileContent(joinFilepath(filepath, index));
			} catch (err) {}
		...
		async handleWorkerRequest(request) {
			...
			try {
				...
				if (body == null) throw new Error("File not found");
		...
		**/
		initPreview() {
			if (!this.initPromise) this.initPromise = this._initPreview();
			return this.initPromise.then((v) => v[0]);
		}
		destroy() {
			if (this.initPromise) {
				this.initPromise.then((val) => {
					val[1].close();
					const url = this.getRelayUrl(val[0]);
					document.body.querySelectorAll(`src="${url}"`).forEach((el) => el.remove());
				});
				this.initPromise = null;
			}
		}
...
var readBuffer = function(content) {
	...
	else return new TextDecoder().decode(content);
};
var validateHtml = function(content) {
	var contentString = readBuffer(content);
	var doc = new DOMParser().parseFromString(contentString, "text/html");
...
var SandpackStatic = function(_super) {
	...
	function SandpackStatic(selector, sandboxSetup, options) {
		...
		_this.previewController = new import_main.PreviewController({
			...
			getFileContent: function(filepath) {
				...
				if (filepath.endsWith(".html") || filepath.endsWith(".htm")) try {
		...
		if (typeof window !== "undefined") window.addEventListener("message", _this.eventListener);
		_this.updateSandbox();
		return _this;
	}
	...
	SandpackStatic.prototype.injectProtocolScript = function(content) {
		return this.injectContentIntoHead(content, "<script>\n  window.addEventListener(\"message\", (message) => {\n    if(message.data.type === \"refresh\") {\n      window.location.reload();\n    }\n  })\n<\/script>");
	};
	SandpackStatic.prototype.injectExternalResources = function(content, externalResources) {
		...
		var tagsToInsert = externalResources.map(function(resource) {
			...
			if (fileType === "css" || resource.includes("fonts.googleapis")) return "<link rel=\"stylesheet\" href=\"".concat(resource, "\">");
			if (fileType === "js") return "<script src=\"".concat(resource, "\"><\/script>");
	...
	*/
	SandpackStatic.prototype.dispatch = function(message) {
		...
		switch (message.type) {
			case "compile":
				...
				break;
			default:
				(_a = this.iframe.contentWindow) === null || _a === void 0 || _a.postMessage(message, "*");
	...
	SandpackStatic.prototype.destroy = function() {
		...
		if (typeof window !== "undefined") window.removeEventListener("message", this.eventListener);
	};
